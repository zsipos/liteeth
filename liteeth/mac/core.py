...
class LiteEthMACCore(Module, AutoCSR):
    def __init__(self, phy, dw, endianness="big",
        ...
        cd_tx_name = phy.crg.cd_eth_tx.name
        cd_rx_name = phy.crg.cd_eth_rx.name
        ...
        self.submodules += ClockDomainsRenamer(cd_tx_name)(tx_gap_inserter)
        ...
        if isinstance(phy, LiteEthPHYModel):
            ...
            self.submodules += ClockDomainsRenamer(cd_tx_name)(preamble_inserter)
            self.submodules += ClockDomainsRenamer(cd_rx_name)(preamble_checker)
            ...
            self.submodules += ClockDomainsRenamer(cd_tx_name)(crc32_inserter)
            self.submodules += ClockDomainsRenamer(cd_rx_name)(crc32_checker)

            tx_pipeline += [preamble_inserter, crc32_inserter]
            rx_pipeline += [preamble_checker, crc32_checker]

            # Error counters
            self.submodules.ps_preamble_error = PulseSynchronizer(cd_rx_name, "sys")
            self.submodules.ps_crc_error = PulseSynchronizer(cd_tx_name, "sys")
        ...
        if with_padding:
            ...
            self.submodules += ClockDomainsRenamer(cd_tx_name)(padding_inserter)
            self.submodules += ClockDomainsRenamer(cd_rx_name)(padding_checker)
        ...
        if dw != 8:
            ...
            self.submodules += ClockDomainsRenamer(cd_tx_name)(tx_last_be)
            self.submodules += ClockDomainsRenamer(cd_rx_name)(rx_last_be)
        ...
        if dw != phy.dw:
            ...
            rx_converter = stream.StrideConverter(eth_phy_description(phy.dw),
                                                  ...
                                                  reverse=reverse)
            self.submodules += ClockDomainsRenamer(cd_tx_name)(tx_converter)
            self.submodules += ClockDomainsRenamer(cd_rx_name)(rx_converter)
        ...
        self.submodules += ClockDomainsRenamer({"write": "sys", "read": cd_tx_name})(tx_cdc)
        self.submodules += ClockDomainsRenamer({"write": cd_rx_name, "read": "sys"})(rx_cdc)
